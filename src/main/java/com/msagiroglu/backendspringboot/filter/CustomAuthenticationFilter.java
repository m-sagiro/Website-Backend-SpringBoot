...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void unsuccessfulAuthentication(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException {
        AuthenticationUtilities.handleAndSendError(response, exception, HttpStatus.UNAUTHORIZED);
    }

    @Override
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authentication) throws IOException {
        User user = (User) authentication.getPrincipal();
        Algorithm algorithm = Algorithm.HMAC256(System.getenv("encSec").getBytes());
        String access_token = AuthenticationUtilities.createAccessToken(request, algorithm, user);
